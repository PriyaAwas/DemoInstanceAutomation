...
// Move From My Profile to Account Infromation
	...
	@FindBy(css = ".toast-message")
	private WebElement lblErrMsg;

	public String getLblErrorMsg() {
		String label = getText(lblErrMsg);
		return label;

	}

	public boolean isErrormsgdisplayed() {
		return isElementVisible(lblErrMsg);
	}

	@FindBy(css = "#toast-container")
	private WebElement dfterrMsg;

	public String getDftErrorMsg() {
		String label = getText(dfterrMsg);
		return label;
	}

	public boolean isDftErrormsgdisplayed() {
		return isElementVisible(dfterrMsg);
	}
...
//		@FindBys(@FindBy(xpath = "#ProfileDivCardHeader0"))
//		private List<WebElement> Myhometile ;
//	
//		public boolean isMyhomelblvisible() {
//			return isElementVisible(Myhometile);
//		}
	@FindBy(css = "#ProfileDivCardHeader0")
	private WebElement Myhometile;
	...
	public boolean isSrvAddTxtVisible() {
		return isElementVisible(srvcaddlbl);
	}

	@FindBy(css = "#ContentPlaceHolder1_ContentPlaceHolderBody_tblViewaccount > li:nth-child(1) > div > div.card_bottom_Sec > div > i:nth-child(1)")
	private WebElement mlgaddlbl;

	public boolean isMlgAddTxtvisible() {
		return isElementVisible(mlgaddlbl);
	}

	@FindBy(css = "#lbldRole0")
	private WebElement rolelbl;

	public boolean isRoleTxtvisible() {
		return isElementVisible(rolelbl);
	}

	@FindBy(css = "#lbldbillType0")
	private WebElement billTypelbl;

	public boolean isBillTypeTxtvisible() {
		return isElementVisible(billTypelbl);
	}

	@FindBy(css = "#divdefaultButton0")
	private WebElement defaultlbl;

	public boolean isDefaultlblvisible() {
		scrollToElement(defaultlbl);
		return isElementVisible(defaultlbl);

	}

	@FindBy(css = ".mysrvc_txt")
	private WebElement acctlndTxt;

	public boolean isAcctlnkdMsgVisible() {
		return isElementVisible(acctlndTxt);
	}

	@FindBy(css = "#titleBanner")
	private WebElement acctInfoBnnr;

	public boolean isAcctInfoBannerVisible() {
		scrollToElement(acctInfoBnnr);
		return isElementVisible(acctInfoBnnr);
	}

	@FindBy(css = "h3[role=\"heading\"]")
	private WebElement fAQ;

	public boolean isFAQBannerVisible() {
		scrollToElement(fAQ);
		return isElementVisible(fAQ);
	}

	@FindBy(css = " .contact_wrp_sidebar")
	private WebElement contactUsInfo;

	public boolean isContactUsVisible() {
		scrollToElement(contactUsInfo);
		return isElementVisible(contactUsInfo);
	}
	...
	public void clickThreeDotIcon() {
		click(btnthreedot);
		log.info("Three dot clicked successfully.");

	}

	@FindBy(css = "#btnNickName0")
	private WebElement editnicname;

	public void clickEditNickNameOption() {
		click(editnicname);
		log.info("Edit Nick Name option clicked successfully.");
	...
	@FindBy(css = "#txtAccountNickName")
	private WebElement txtAccountNickName;

	public boolean isAccountNickNameTxtBoxMessageBodyVisible() {
		waitForElementToBeVisible(textAccountNickName);
		return textAccountNickName.isDisplayed();
	}

	public void populateTxtMessageBody(String Text) {
		log.info("Waiting For Visiblity status of MessageBody Text Feild");
		waitForElementToBeVisible(textAccountNickName);
		clear(textAccountNickName);
		sendKeys(textAccountNickName, Text);
	}

	public WebElement elementTextMessageBody() {
		waitForElementToBeVisible(textAccountNickName);
		return textAccountNickName;
	}

	@FindBy(css = "#btnAccountUnlink0")
	private WebElement unlinkAcc;

	public void clickUnlinkAcc() {
		click(unlinkAcc);
		log.info("Unlink Button clicked successfully.");
	...
	public boolean editNickNameWarningToastDisplayed() {
		return isElementVisible(lablMsgWarning);
	}

	public String getEditNickNameWarningMessage() {
		pause(5000);
		String label = getText(lablMsgWarning);
		return label;
	}
	...
	public void clickCtnBtn() {
		click(ctnBtn);
		log.info("Continue clicked successfully.");
	}

	@FindBy(css = "#btnEnroll0")
	private WebElement editBilltype;

	public void clickEditBillType() {
		click(editBilltype);
		log.info("Edited Bill Type successfully.");
	}

	@FindBy(css = "#lblRdnPaperBoth")
	private WebElement radiobtn;

	public void clickBothrdbtn() {
		pause(3000);
		click(radiobtn);
		log.info("Both Radio Btn clicked successfully.");
	}
