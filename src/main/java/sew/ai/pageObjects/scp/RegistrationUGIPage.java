...
public class RegistrationUGIPage extends LoginPage {

    private static final Logger log = LogManager.getLogger(RegistrationUGIPage.class);

    public RegistrationUGIPage(WebDriver driver) {
        super(driver);
    ...
    public String getAtrributeFirstName(String attribute) {
    ...
    private WebElement txtUtilityAccountNumber;

    public void populateTextUtilityAccountNumber(String UtilityAccountNumber) {
        log.info("Populating First Name {} :" + UtilityAccountNumber);
        sendKeys(txtUtilityAccountNumber, UtilityAccountNumber);
        log.info("UtilityAccountNumber populated successfully.");
    }

    @FindBy(css = "[globalize='ML_Registration_Correspondence'] input")
    private WebElement chkEmailCorrespondance;

    public boolean isCheckBoxEmailCorrespondanceChecked() {
        return isElementSelected(chkEmailCorrespondance);
    }

    public void checkChkBoxEmailCorrespondance() {
        Page.scrollToTheBottomOfPage();
        check(chkEmailCorrespondance);
        log.info("Terms and conditions checkbox button is successfully.");
    }

    public boolean isChkBoxEmailCorrespondanceVisible() {
        Page.scrollToTheBottomOfPage();
        log.info("Checking the visibility of terms and conditions on the page.");
        log.info("First Name visibility status : " + isElementVisible(chkEmailCorrespondance));
        return isElementVisible(chkEmailCorrespondance);
    }

    @FindBy(css = " span[globalize = 'ML_Registration_PaperlessBill'] input")
    private WebElement chkBoxReceivePaperBill;

    public WebElement verifyCheckBoxReceivePaperBill() {
        return chkBoxReceivePaperBill;
    }

    public boolean isCheckBoxReceivePaperBillChecked() {
        return isElementSelected(chkBoxReceivePaperBill);
    }

    public void checkChkBoxReceivePaperBill() {
        Page.scrollToTheBottomOfPage();
        check(chkBoxReceivePaperBill);
        log.info("Terms and conditions checkbox button is successfully.");
    }

    public boolean isChkBoxReceivePaperBillVisible() {
        Page.scrollToTheBottomOfPage();
        log.info("Checking the visibility of terms and conditions on the page.");
        log.info("First Name visibility status : " + isElementVisible(chkBoxReceivePaperBill));
        return isElementVisible(chkBoxReceivePaperBill);
    }
    ...
    public String gettxtMfaOtpTimer() {
        return getText(txtMfaOtpTimer);
    }

    @FindBy(css = "#bankchoseopt")
    private WebElement btnRegisterCommercial;

    public void clickRegisterCommercialBtn() {
        click(btnRegisterCommercial);
        log.info("RegisterCommercial Button clicked successfully.");
    }

    @FindBys(@FindBy(css = "input#NextBtn.submit-button"))
    private List < WebElement > btnRegister;

    public void RegisterBtn() {
        waitForElementToBeClickable(btnRegister.get(0));
        click(btnRegister.get(0));
        log.info("Register Button clicked successfully.");
    }

    public boolean isRegisterBtnVisible() {
        if (btnRegister.size() > 0) {
            return isElementVisible(btnRegister.get(0));
        } else {
            return false;
        }
    }
    ...
    public boolean isSubmitButtonEnable() {
        ...
        log.info("Submit button enable status : " + isElementEnabled(btnRegister.get(0)));
        return isElementEnabled(btnRegister.get(0));
    }
    ...
    public boolean isButtonPreviousisVisible() {
        log.info("Checking the visibility of Previous Button on the page.");
        log.info("First Name visibility status : " + isElementVisible(btnPrevious));
    ...
    @FindBy(css = "#NextBtn.registration_btn")
    private WebElement btnGuestRegistrationNext;

    public void clickGuestRegistrationNext() {
        scrollToElement(btnGuestRegistrationNext);
        clickElementUsingJsExecutor(btnGuestRegistrationNext);
        log.info("Guest registration Next Button clicked successfully.");
    }

    public boolean isGuestRegistrationNextVisible() {
        return isElementVisible(btnGuestRegistrationNext);
    }
    ...
    @FindBy(css = ".cancel-button.cancel_button_comm.stback2.mfaback2")
    private WebElement btnRegCancel;

    public boolean isbtnRegCancelVisible() {
        return isElementVisible(btnRegCancel);
    }

    public void clickbtnRegCancel() {
        scrollToElement(btnRegCancel);
        clickElementUsingJsExecutor(btnRegCancel);
        log.info("btnRegCancel clicked successfully.");
    }
    ...
    public boolean verifyCheckBoxTermsConditions() {
        return isElementSelected(chkBoxTermsConditions);
    }
    ...
    public boolean verifySubscribeForNotification() {
        return isElementSelected(chkBoxNotificationPref);
    }

    public void checkSubscribeForNotification() {
        check(chkBoxNotificationPref);
    ...
    public String getlnkRegResendOtp(String value) {
        return getAttribute(lnkRegResendOtp, value);
    }

    public void clicklnkRegResendOtp() {
        click(lnkRegResendOtp);
    }
    ...
    public void clickChkPhoneNumberTermandCon() {
        scrollToElement(chkPhoneNumberTermandCon.get(0));
        click(chkPhoneNumberTermandCon.get(0));
    }
    ...
    public String getLblverificationReceivedmsg() {
        return getText(lblverificationReceivedmsg);
    }
    ...
    public boolean isCheckResidentialRdoSelected() {
        boolean isSelected = rdbResidential.isSelected();
        return isSelected;
    }
    ...
    public void checkCommercialRdo() {
        if (!rdbCommercial.isSelected()) {
            check(rdbCommercial);
        }
        // waitForPageLoader();
        log.info("Commercial Radio button is successfully.");
    }
    ...
    // @FindBy(css = "[id*=pnlLevel] .error_messagecommon")
    @FindBy(css = ".toast-message")
    ...
    public void waitForLblRegistrationErrorMessageToBeVisible() {
        waitForElementToBeVisible(lblRegistrationErrormessage);
        log.info("Email Address Validation Message is visible.");

    }

    @FindBy(css = "#pnlLevel1 .error_messagecommon")
    private WebElement lblRegistrationErrormessage1;

    public String getLblRegistrationErrormessage1() {
        log.info("Fetching the registration page error message.");
        String label = getText(lblRegistrationErrormessage1);
        log.info("Registration page header is {}: " + label);
        return label;
    }

    // Priya
    @FindBy(css = "#successHeader")
    private WebElement header_emailid;

    public String getEmailID() {
        log.info("Fetching the EmailID Header.");
        String label = getText(header_emailid);
        log.info("EmailID Header {}: " + label);
        return label;
    }

    @FindBy(css = "#txt_95")
    private WebElement txtBoxUserId;

    public void populateUserId(String UserId) {
        log.info("Populating UserId {} :" + UserId);
        sendKeys(txtBoxUserId, UserId);
        log.info("UserId populated successfully.");
    }
